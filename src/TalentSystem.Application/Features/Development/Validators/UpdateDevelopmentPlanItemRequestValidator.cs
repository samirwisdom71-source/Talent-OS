...
public sealed class UpdateDevelopmentPlanItemRequestValidator : AbstractValidator<UpdateDevelopmentPlanItemRequest>
{
    public UpdateDevelopmentPlanItemRequestValidator()
    {
        ...
        RuleFor(x => x.Status)
            .Must(s => Enum.IsDefined(typeof(DevelopmentItemStatus), s))
            .WithMessage(
                "Status must be a defined development item status (1=NotStarted, 2=InProgress, 3=Completed, 4=Cancelled). " +
                "Omitting status in JSON defaults to 0, which is invalid.");
